...
def load_data():
    ...
    cursor.execute("""
        SELECT name, mass_gev, category, charge, spin_half, isospin_z
        FROM particles WHERE mass_gev > 0
    """)
    
    particles = []
    for name, mass, category, charge, spin_half, isospin in cursor.fetchall():
        particles.append({
            'name': name, 'mass': mass, 'category': category,
            'q': exact_q[name], 'n': exact_q[name]/4,
            'charge': charge, 'spin': spin_half*0.5 if spin_half else 0,
            'isospin': isospin
        })
